...
# 实例用例与测试数据
excel = Excel(BASE_PATH + f'\Reports\源码检测报告.xlsx', BASE_PATH+f'\casedata\供应链场景excel报告测试.xlsx', "sheet标题及表头测试")
...
class TestSheet:
    ...
    def test_sheets_title(self):
        try:
            case_sheets = excel.read_caseData('源码检测', 'sheet标题')
            if case_sheets:
                ...
                if case_sheets != report_sheets:
                    ...
                    if len(diff) != 0:
                        log.error(f"源码检测报告-sheet标题与用例数据不匹配！diff:{diff}")
                        assert False, f"数据不匹配-diff:{diff}"
                    else:
                        log.error(f"源码检测报告-sheet标题与用例顺序不匹配！")
